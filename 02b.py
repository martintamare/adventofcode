...
for item in data:
    ...
    index_yes, index_no, letter, password = m.groups()
    index_yes = int(index_yes) - 1
    index_no = int(index_no) - 1

    max_index = len(password) -1
    if max_index < index_yes:
        print(f'skipping {item} index_yes')
        exit(0)
        continue
    if max_index < index_no:
        print(f'skipping {item} index_no')
        exit(0)
        continue

    count = 0
    if password[index_yes] == letter:
        count += 1
    if password[index_no] == letter:
        count += 1
