...
def solve(data, iteration):
    ...
    for i in range(iteration):
        ...
        # First get unique neighbors meaning can be outside of current matrix
        next_plots = set()
        ...
        # Set transform to the initial_matrix
        final_set = []
        for row, col in next_plots:
            final_row = row % garden.rows
            final_col = col % garden.cols
            final_vector = (final_row, final_col)
            final_set.append(final_vector)
            garden.set_visited(final_row, final_col, True)

        # Counter value
        final_counter = Counter(final_set)
        new_plots_counter = {}
        for vector, number in final_counter.items():
            if vector in plots_counter:
                current = plots_counter[vector]
                new_plots_counter[vector] = current + number
            else:
                new_plots_counter[vector] = number
        plots_counter = new_plots_counter
        current_plots = final_set

        print(garden)
        print(set(final_counter.values()))
        print(plots_counter)
        current_sum = sum(plots_counter.values())
        input(f"{i+1} {current_sum=}")
    return sum(plots_counter.values())
    pass
